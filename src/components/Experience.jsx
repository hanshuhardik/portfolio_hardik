...
const Experience = () => {
  return (
    <div className="border-b border-neutral-900 my-20 pd-4">
        ...
        className="my-200 text-center pb-5 text-4xl">Experience</motion.h1>
        <div>
            {EXPERIENCES.map((experience,index)=>(
                <div key={index} className="mb-8 flex flex-wrap lg:justify-center">
                    ...
                    {experience.technologies.map((tech,index)=>(
                        <span className='mr-2 mt-4 rounded bg-neutral-900 px-2 py-1 text-sm font-mediun text-purple-800' key={index} >{tech}</span>
                    ))}
                    </motion.div>
                    </div>
            ))}
